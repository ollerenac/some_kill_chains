#!/usr/bin/env python3
"""Generate PDF from SCENARIOS.md using reportlab."""
...
def parse_md(text):
    ...
    while i < len(lines):
        ...
        # H3 — scenario or Referencias (still Spanish in source file)
        if line.startswith("### "):
            header = line[4:].strip()
            ...
            cur_scenario = {
                ...
                "killchain": "",
                "refs": [],
                "multi": "Multi-step" in header or "2 flags" in header,
            }
            state = "scenario"
            i += 1
            continue
        ...
        if state == "scenario" and cur_scenario is not None:
            if line.startswith("**Difficulty:**"):
                cur_scenario["difficulty"] = line.split("**Difficulty:**")[-1].strip()
        ...
        # Inside references table (headers still in Spanish: Recurso / URL / Propósito)
...
def ref_table(refs):
    if not refs:
        return []
    hdr = [
        Paragraph("Resource", S["ref_hdr"]),
        Paragraph("URL", S["ref_hdr"]),
        Paragraph("Purpose", S["ref_hdr"]),
    ]
    rows = [hdr]
...
def cover_page(doc_title):
    ...
    usable_w = W - 3.4*cm
    items = [
        LogoBanner(usable_w, "CTF Scenario Catalog",
                   "Advanced Cybersecurity Lab"),
        Spacer(1, 1.2*cm),
        Paragraph("23 proposed scenarios · 6 attack domains", S["doc_sub"]),
        Spacer(1, 0.5*cm),
        Paragraph("Active Directory · Network Protocols · CVE · Cloud · LLM · APT", S["doc_sub"]),
        Spacer(1, 2*cm),
        HRFlowable(width="100%", thickness=1, color=C_RED),
        Spacer(1, 0.4*cm),
        Paragraph("Scenario Proposal — Public Version", S["intro"]),
        PageBreak(),
    ]
    return items
...
def build_pdf(src_path: Path, out_path: Path):
    ...
    doc = SimpleDocTemplate(
        ...
        title="CTF Scenario Catalog",
        author="Advanced Cybersecurity Lab",
    )
    ...
    for domain in domains:
        ...
        for sc in domain["scenarios"]:
            ...
            if sc["refs"]:
                story.append(Paragraph("References", S["ref_section"]))
...
if __name__ == "__main__":
    ...
    src = repo / "SCENARIOS.md"
    out = repo / "propuesta_escenarios.pdf"
    if len(sys.argv) > 1:
        out = Path(sys.argv[1])
    build_pdf(src, out)
